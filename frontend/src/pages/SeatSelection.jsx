...
export default function SeatLayout() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setFetchError(err.message || 'Failed to load seat layout');
            } finally {
                setLoading(false);
            }
        };
    ...
    if (fetchError || !layoutData) return (
        <div className="min-h-screen bg-[#0f172a] flex items-center justify-center">
            <div className="text-center max-w-md px-6">
                <div className="text-5xl mb-4">⚠️</div>
                <h2 className="text-xl font-bold text-white mb-2">Could not load seat layout</h2>
                <p className="text-gray-400 mb-6">{fetchError || 'No seat data available for this train.'}</p>
                <button onClick={() => navigate(-1)} className="px-6 py-3 rounded-xl font-bold text-gray-900 bg-gray-100 hover:bg-white transition">← Go Back</button>
            </div>
        </div>
    );
