...
export default function SeatLayout() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (trainInfo?.runningDays?.length > 0) {
        ...
        fetchData();
    }, [trainNumber, classType, journeyDate]);
    ...
    const isUnreservedClass = ['GN', 'GS', 'UR'].includes(classType?.toUpperCase()) ||
    ...
    if (!trainRunsOnDate) return (
        <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center text-center p-6">
            <div className="w-20 h-20 bg-gray-800 rounded-full flex items-center justify-center mb-6"><span className="text-4xl">🛑</span></div>
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">Train Does Not Run</h2>
            <p className="text-gray-400 max-w-md">{trainDetails?.trainName || `Train #${trainNumber}`} does not run on {new Date(journeyDate).toLocaleString('en-US', { weekday: 'long' })}s.</p>
            <button onClick={() => navigate(-1)} className="mt-8 px-6 py-3 bg-[#4ab86d] text-white font-bold rounded-xl hover:bg-green-600 transition shadow-lg">Go Back</button>
        </div>
    );
            ...
            <div className="max-w-6xl mx-auto">
                ...
                {isTrainSearchMode ? (
                    <div className="bg-[#1D2332] rounded-2xl md:rounded-3xl p-5 sm:p-6 md:p-8 mb-6 shadow-2xl border border-white/5 relative overflow-hidden group">
                        ...
                        <div className="absolute -top-32 -right-32 w-64 h-64 bg-[#4ab86d]/10 rounded-full blur-[60px] pointer-events-none"></div>
